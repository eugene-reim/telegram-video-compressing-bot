...
async def process_video(
    ...
    *,
    ...
    # If another job is already running, announce the queue first,
    # then wait so we don't spam Telegram with parallel status edits.
    ...
    async with compress_semaphore:
        try:
            ...
            await _safe_edit(status_msg, "⬇️ Скачиваю видео…")
            tg_file = await context.bot.get_file(file_id)
            await tg_file.download_to_drive(custom_path=str(input_path))

            if not input_path.exists() or input_path.stat().st_size == 0:
                await _safe_edit(status_msg, "❌ Не удалось скачать видео.")
                return
            ...
            if not success or not output_path.exists():
                await _safe_edit(status_msg, "❌ Сжатие не удалось. Проверьте логи.")
                return
            ...
            try:
                ...
                await _safe_edit(status_msg, "✅ Готово.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "/сtatus — Показать настройки и лимиты\n"
        "/help — Это сообщение\n\n"
